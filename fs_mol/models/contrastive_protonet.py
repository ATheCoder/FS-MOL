...
def calculate_mahalanobis_logits(support_features_flat, support_labels, query_features_flat, device=None):
    ...
    wandb.log({"class_means": wandb.Histogram(class_means.cpu().detach().numpy())})
    wandb.log({"class_precision_matrices": wandb.Histogram(class_precision_matrices.cpu().detach().numpy())})
    ...
    wandb.log({"repeated_target": wandb.Histogram(repeated_target.cpu().detach().numpy())})
    repeated_class_means = class_means.repeat(number_of_targets, 1)
    repeated_difference = repeated_class_means - repeated_target
    wandb.log({"repeated_difference_1": wandb.Histogram(repeated_difference.cpu().detach().numpy())})
    repeated_difference = repeated_difference.view(
        number_of_targets, number_of_classes, repeated_difference.size(1)
    ).permute(1, 0, 2)
    wandb.log({"repeated_difference_2": wandb.Histogram(repeated_difference.cpu().detach().numpy())})
...
class PrototypicalNetwork(nn.Module):
    ...
    def forward(self, input_batch: ProtoNetBatch):
        ...
        if self.config.distance_metric == "mahalanobis":
            return calculate_mahalanobis_logits(support_features_flat, input_batch.support_labels, query_features_flat, device=self.device)
