...
class MRCLightningModule(L.LightningModule):
    ...
    def calc_loss(self, input):
        ...
        # batch = torch.nan_to_num(batch, 0)
...
if config.isProd:
    wandb.init(project=f"{config.representation}_molecular_representation_comparison_final", config=config.__dict__, resume=True, id='c5i8is38')
...
trainer.fit(model=model, train_dataloaders=train_dl, val_dataloaders=valid_dls, ckpt_path="/FS-MOL/MRC_Runner/last-v80.ckpt")
