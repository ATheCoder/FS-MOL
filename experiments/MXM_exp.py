...
train_dataset = FSMOLHTorchDataset("train", "pyg", config.train_n_repeats)
valid_dataset = FSMOLHTorchDataset("valid", "pyg", config.val_n_repeats)
...
trainer = L.Trainer(callbacks=[checkpoint_callback], check_val_every_n_epoch=1, max_epochs=100, logger=WandbLogger(), accumulate_grad_batches=config.accumulate_grad_batches)
