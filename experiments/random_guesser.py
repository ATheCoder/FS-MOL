...
class MRCLightningModule(L.LightningModule):
    ...
    def calc_loss(self, input):
        _, labels, is_query, _, task_names = input
        num_queries = is_query.sum().item()

        # Generate random predictions between 0 and 1
        logits = torch.rand(num_queries, dtype=torch.float32, device=self.device)
        batch_labels = labels[is_query == 1]

        # Dummy loss calculation (no training effect)
        loss = torch.nn.functional.binary_cross_entropy_with_logits(logits, batch_labels.float())

        return loss, logits, batch_labels, task_names
    ...
    def validation_step(self, batches):
        ...
        batch_preds = torch.sigmoid(logits).cpu()

        # Compute metrics
        metrics = compute_binary_task_metrics(
            predictions=batch_preds, labels=query_labels.detach().cpu().numpy()
        )
...
if config.isProd:
    wandb.init(project="molecular_representation_comparison_random_baseline", config=config)
